const DropdownField = ({ label, name, value, onChange, error, onBlur, options }: { label: string, name: string, value: string, onChange: (e: React.ChangeEvent<HTMLSelectElement>) => void, error: string, onBlur: (e: React.FocusEvent<HTMLSelectElement>) => void, options: string[] }) => {
    return (
        <div className="relative w-full max-w-md mb-8">
            <select name={name} value={value} onChange={onChange} className="block w-full p-2 text-sm text-gray-900 rounded-xl text-gray-900 border border-gray-300 focus:border-rose-300 peer focus:ring-1 focus:ring-amber-500 focus:outline-none" onBlur={onBlur}>
                {options.map((option) => (
