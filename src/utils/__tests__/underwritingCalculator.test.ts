...
describe('Underwriting Calculator - CSV Validation', () => {
  describe('CSV Test Case 1 - Score 16 (Medium-High Risk)', () => {
    it('should match CSV example for Venue/Other: 1-2 years, 7-10 events, Ticketing Co, Weekly', () => {
      ...
      expect(result!.maxAdvancePercent).toBe(0.08); // 8% for score 16 (14.1-21 band, V/O column)
      expect(result!.advanceAmount).toBe(200000); // $2.5M * 8% = $200k
      expect(result!.isCapped).toBe(false);
    });
  });

  describe('CSV Test Case 2 - Score 9 (Low-Medium Risk)', () => {
    it('should match CSV example for Promoter: 6-9 years, 21+ events, Ticketing Co, Weekly', () => {
      const inputs: UnderwritingInputs = {
      ...
      expect(result!.maxAdvancePercent).toBe(0.15); // 15% for score 9 (7.1-14 band, P column)
      expect(result!.advanceAmount).toBe(150000); // $1M * 15% = $150k
      expect(result!.isCapped).toBe(false);
    });
  });

  describe('CSV Test Case 3 - Score 2 (Low Risk)', () => {
    it('should match CSV example for Festival: 10+ years, 50+ events, Ticketing Co, Weekly', () => {
      const inputs: UnderwritingInputs = {
      ...
      expect(result!.maxAdvancePercent).toBe(0.26); // 26% for score 2 (0-7 band, F column)
      expect(result!.advanceAmount).toBe(260000); // $1M * 26% = $260k
      expect(result!.isCapped).toBe(false);
    });
  });

  describe('CSV Test Case 4 - Score 23 (High Risk)', () => {
    it('should match CSV example for Venue/Other: 6-9 years, 11-20 events, Other, Post event', () => {
      const inputs: UnderwritingInputs = {
      ...
      expect(result!.maxAdvancePercent).toBe(0.05); // 5% for score 23 (21.1-35 band)
      expect(result!.advanceAmount).toBe(90000); // $1.8M * 5% = $90k
      expect(result!.isCapped).toBe(false);
    });
  });
  ...
  describe('Cap Test - $500k Maximum', () => {
    it('should apply $500k cap for very high ticket sales', () => {
      ...
      expect(result!.maxAdvancePercent).toBe(0.26); // 26%
      // Raw amount would be $20M * 26% = $5.2M, but capped at $500k
  ...
  describe('Risk Band Tests', () => {
    it('should calculate 14% for V/O low risk (score 0-7)', () => {
      const inputs: UnderwritingInputs = {
      ...
      expect(result!.maxAdvancePercent).toBe(0.14);
    });

    it('should calculate 11% for V/O medium-low risk (score 7.1-14)', () => {
      const inputs: UnderwritingInputs = {
      ...
      expect(result!.maxAdvancePercent).toBe(0.11);
    });

    it('should calculate 8% for V/O medium-high risk (score 14.1-21)', () => {
      const inputs: UnderwritingInputs = {
      ...
      expect(result!.maxAdvancePercent).toBe(0.08);
    });

    it('should calculate 5% for high risk (score 21.1-35)', () => {
      const inputs: UnderwritingInputs = {
        yearsInBusiness: 'Less than 1 year', // 10 pts
  ...
  describe('Edge Cases', () => {
    ...
    it('should handle boundary conditions in risk matrix', () => {
      ...
      expect(result1!.maxAdvancePercent).toBe(0.14); // Should be in 14% band (0-7, V/O)
      ...
      expect(result2!.maxAdvancePercent).toBe(0.11); // Should be in 11% band (7.1-14, V/O)
    });
  });
  ...
  describe('Form Key Mapping Tests - Validates mapping from form keys to values', () => {
     ...
     * - paymentProcessing: 'Ticketing Co' -> 'My Ticketing Co'
    ...
    it('should work with form values after mapping (not form keys)', () => {
      ...
      const inputs: UnderwritingInputs = {
        ...
        numberOfEvents: 1,
        paymentRemittedBy: 'My Ticketing Co', // Mapped from 'Ticketing Co'
      ...
      expect(result!.breakdown.paymentRemittedByScore).toBe(1); // Alias should resolve to canonical score
    ...
    it('should apply the Festival column when customer type is Festival', () => {
      const inputs: UnderwritingInputs = {
        yearsInBusiness: '10+ years',
        numberOfEvents: 50,
        paymentRemittedBy: 'My Ticketing Co',
        paymentFrequency: 'Daily',
        grossAnnualTicketSales: 1000000,
        customerType: 'Festival'
      };
      ...
      expect(result).not.toBeNull();
      expect(result!.totalRiskScore).toBe(1);
      expect(result!.maxAdvancePercent).toBe(0.26);
      expect(result!.advanceAmount).toBe(260000);
    });
  });
});
