...
test("prod form critical path stays healthy", async ({ page }) => {
  ...
  const formWebhookUrl = "https://webhook.test.com/test";
  const fileUploadWebhookUrl = "https://hook.us1.make.com/test-upload";
  ...
  await page.route(formWebhookUrl, async (route) => {
    await route.fulfill({
      status: 200,
  ...
  await page.route(fileUploadWebhookUrl, async (route) => {
    await route.fulfill({
      status: 200,
      contentType: "application/json",
      body: JSON.stringify({ ok: true }),
  ...
  const waitForUploadPost = () =>
    page.waitForResponse(
      (response) =>
        ...
        response.url() === fileUploadWebhookUrl,
      { timeout: 30_000 }
    );
